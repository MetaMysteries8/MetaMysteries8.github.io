...
(() => {
  ...
  const state = {
    ...
    renderVersion:1, loading:false, renderedDurations:[], autosaveTimer:0, autosaveReady:false, dragId:null, playingLineId:null
  };
  const CONV_AUTOSAVE_KEY='mainvoice.conversation.autosave.v2';
  ...
  function setAutosaveStatus(t){const e=$('convAutosaveStatus');if(e)e.textContent=t}
  function scheduleAutosave(){if(!state.autosaveReady)return;clearTimeout(state.autosaveTimer);state.autosaveTimer=setTimeout(()=>{try{const p=project();p._autosavedAt=Date.now();localStorage.setItem(CONV_AUTOSAVE_KEY,JSON.stringify(p));setAutosaveStatus('Autosaved locally · '+new Date(p._autosavedAt).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit',second:'2-digit'}))}catch(e){setAutosaveStatus('Autosave unavailable: '+e.message)}},400)}
  function invalidate(){state.rendered=null;state.renderedStarts=[];state.renderedDurations=[];state.lineCache.clear();state.renderVersion++;renderChatPreview();scheduleAutosave()}
  ...
  function applyStyleToSpeaker(id){const src=state.lines.find(l=>l.id===id);if(!src)return;let count=0;for(const l of state.lines){if(l.id===src.id||l.voiceId!==src.voiceId)continue;for(const k of ['expression','expressionAmount','pitch','speed','gainDb','pan'])l[k]=src[k];count++}invalidate();renderLines();msg(`Applied ${voiceById(src.voiceId)?.name||'speaker'} delivery to ${count} other line${count===1?'':'s'}.`,'good')}
  ...
  function renderLines(){
    root.innerHTML='';root.classList.toggle('compact',$('convCompact')?.checked);$('convEmpty').hidden=!!state.lines.length;
    state.lines.forEach((l,i)=>{
      ...
      el.innerHTML=`
        <div class="conversation-line-handle"><div class="conversation-drag-grip" draggable="true" title="Drag to reorder">⋮⋮</div><div class="conversation-line-number">${i+1}</div><button class="conversation-move" data-act="up" title="Move up">↑</button><button class="conversation-move" data-act="down" title="Move down">↓</button></div>
        ...
        <div class="conversation-line-body">
          <div class="conversation-line-top">
            ...
            <div class="conversation-line-actions"><button class="button primary conv-preview">▶ Line</button><button class="button secondary" data-act="play-from">▶ From here</button><button class="button secondary" data-act="duplicate">Duplicate</button><button class="button secondary" data-act="apply-style" title="Copy this delivery to every line using the same speaker">Apply style</button><button class="button secondary" data-act="add-after">+ Below</button><button class="button danger" data-act="delete">Delete</button></div>
          </div>
      ...
      const textArea=el.querySelector('.conv-text');const autoSize=()=>{textArea.style.height='auto';textArea.style.height=Math.min(260,Math.max(78,textArea.scrollHeight))+'px'};textArea.oninput=e=>{update('text',e.target.value);autoSize()};textArea.onkeydown=e=>{if((e.ctrlKey||e.metaKey)&&e.key==='Enter'){e.preventDefault();playSingle(l.id)}else if(e.altKey&&e.key==='Enter'){e.preventDefault();addLine(l.id)}};setTimeout(autoSize,0);
      ...
      el.querySelectorAll('[data-act]').forEach(b=>b.onclick=()=>{const a=b.dataset.act;if(a==='up')moveLine(l.id,-1);if(a==='down')moveLine(l.id,1);if(a==='play-from')playFrom(l.id);if(a==='duplicate')duplicateLine(l.id);if(a==='apply-style')applyStyleToSpeaker(l.id);if(a==='add-after')addLine(l.id);if(a==='delete')deleteLine(l.id)});
      const grip=el.querySelector('.conversation-drag-grip');grip?.addEventListener('dragstart',e=>{state.dragId=l.id;e.dataTransfer.effectAllowed='move';e.dataTransfer.setData('text/plain',String(l.id));el.classList.add('dragging')});grip?.addEventListener('dragend',()=>{state.dragId=null;el.classList.remove('dragging');document.querySelectorAll('.conversation-line.drag-over').forEach(x=>x.classList.remove('drag-over'))});el.addEventListener('dragover',e=>{if(state.dragId&&state.dragId!==l.id){e.preventDefault();e.dataTransfer.dropEffect='move';el.classList.add('drag-over')}});el.addEventListener('dragleave',()=>el.classList.remove('drag-over'));el.addEventListener('drop',e=>{e.preventDefault();el.classList.remove('drag-over');const from=state.lines.findIndex(x=>x.id===state.dragId),to=state.lines.findIndex(x=>x.id===l.id);if(from<0||to<0||from===to)return;const [moved]=state.lines.splice(from,1);state.lines.splice(to,0,moved);invalidate();renderLines();msg('Dialogue line reordered.','good')});root.appendChild(el);
    });
    renderChatPreview();
  }
  ...
  async function buffersForVoice(v){
    if(app.getVoiceBuffers)return app.getVoiceBuffers(v);if(state.bufferCache.has(v.id))return state.bufferCache.get(v.id);
    const ctx=app.getAudioCtx();const pairs=await Promise.all(app.phonemes.map(async p=>{try{let ab;if(v.temporary&&v._sampleBytes){ab=v._sampleBytes.get(p.c);if(!ab)return[p.c,null];ab=ab.slice(0)}else{const r=await fetch(`${v.base_url}/samples/${p.c}.wav`,{cache:'no-store'});if(!r.ok)return[p.c,null];ab=await r.arrayBuffer()}return[p.c,await ctx.decodeAudioData(ab.slice(0))]}catch{return[p.c,null]}}));const m=new Map(pairs.filter(x=>x[1]));state.bufferCache.set(v.id,m);return m;
  }
  ...
  async function renderConversation(){
    ...
    rendered.forEach((r,i)=>{const s=off.createBufferSource(),g=off.createGain();s.buffer=r.buffer;s.connect(g);g.gain.value=1;let last=g;if(off.createStereoPanner){const p=off.createStereoPanner();p.pan.value=clamp(r.line.pan/100,-1,1);g.connect(p);last=p}last.connect(master);s.start(starts[i])});const mix=await off.startRendering();state.rendered=mix;state.renderedStarts=starts;state.renderedDurations=rendered.map(x=>x.buffer.duration);state.loading=false;msg(`Ready — ${usable.length} lines, ${mix.duration.toFixed(2)} seconds.`,'good');return mix;
  }

  function stop(){if(state.source){try{state.source.stop()}catch{}state.source=null}cancelAnimationFrame(state.raf);state.raf=0;state.playingLineId=null;document.querySelectorAll('.conversation-line.playing').forEach(e=>e.classList.remove('playing'));app.stopSpeech?.()}
  function playBuffer(buf,startMap=null,startOffset=0){stop();const ctx=app.getAudioCtx(),s=ctx.createBufferSource();s.buffer=buf;s.connect(ctx.destination);s.start(0,Math.max(0,startOffset));state.source=s;const started=performance.now(),usable=state.lines.filter(l=>String(l.text).trim());const tick=()=>{if(!state.source)return;const sec=startOffset+(performance.now()-started)/1000;if(startMap?.length){let idx=0;for(let i=0;i<startMap.length;i++)if(startMap[i]<=sec)idx=i;document.querySelectorAll('.conversation-line').forEach(e=>e.classList.remove('playing'));const line=usable[idx],node=line?document.querySelector(`[data-conv-line="${line.id}"]`):null;if(node){node.classList.add('playing');if(state.playingLineId!==line.id){state.playingLineId=line.id;node.scrollIntoView?.({block:'nearest',behavior:'smooth'})}}}state.raf=requestAnimationFrame(tick)};tick();s.onended=()=>{if(state.source===s)state.source=null;cancelAnimationFrame(state.raf);state.raf=0;document.querySelectorAll('.conversation-line.playing').forEach(e=>e.classList.remove('playing'))}}
  ...
  async function playFrom(id){const usable=state.lines.filter(l=>String(l.text).trim()),idx=usable.findIndex(l=>l.id===id);if(idx<0)return;try{$('convPlayBtn').disabled=true;const b=await renderConversation(),offset=state.renderedStarts[idx]||0;playBuffer(b,state.renderedStarts,offset);msg(`Playing from line ${state.lines.findIndex(x=>x.id===id)+1}.`,'good')}catch(e){state.loading=false;msg(e.message,'bad')}finally{$('convPlayBtn').disabled=false}}
  ...
  function exportTranscript(){
    const rows=state.lines.filter(l=>String(l.text).trim()).map(l=>`${voiceById(l.voiceId)?.name||'Missing voice'}: ${l.text.trim()}`);
    if(!rows.length){msg('There is no dialogue to export.','bad');return}
    app.download(new Blob([rows.join('\n\n')+'\n'],{type:'text/plain'}),cleanName($('convTitle').value)+'.txt');msg('Transcript exported.','good')
  }
  function srtTime(sec){sec=Math.max(0,sec);const ms=Math.round(sec*1000),h=Math.floor(ms/3600000),m=Math.floor(ms%3600000/60000),s=Math.floor(ms%60000/1000),z=ms%1000;return `${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')},${String(z).padStart(3,'0')}`}
  async function exportSrt(){
    try{
      const usable=state.lines.filter(l=>String(l.text).trim());if(!usable.length)throw new Error('There is no dialogue to subtitle.');await renderConversation();
      const blocks=usable.map((l,i)=>`${i+1}\n${srtTime(state.renderedStarts[i]||0)} --> ${srtTime((state.renderedStarts[i]||0)+(state.renderedDurations[i]||.2))}\n${voiceById(l.voiceId)?.name||'Voice'}: ${l.text.trim()}`);
      app.download(new Blob([blocks.join('\n\n')+'\n'],{type:'application/x-subrip'}),cleanName($('convTitle').value)+'.srt');msg('SRT subtitles exported from rendered line timings.','good')
  ...
  function populateSpeakerTools(){for(const id of ['convReplaceFrom','convReplaceTo']){const sel=$(id);if(!sel)continue;const old=sel.value;sel.innerHTML=optionHtml(old);if(old&&app.voices.some(v=>v.id===old))sel.value=old}if($('convReplaceFrom')&&!$('convReplaceFrom').value)$('convReplaceFrom').value=app.voices[0]?.id||'';if($('convReplaceTo')&&!$('convReplaceTo').value)$('convReplaceTo').value=app.voices[1]?.id||app.voices[0]?.id||''}
  function replaceSpeaker(swap=false){const a=$('convReplaceFrom')?.value,b=$('convReplaceTo')?.value;if(!a||!b||a===b){msg('Choose two different voices first.','bad');return}let count=0;for(const l of state.lines){if(l.voiceId===a){l.voiceId=b;count++}else if(swap&&l.voiceId===b){l.voiceId=a;count++}}invalidate();renderLines();msg(`${swap?'Swapped':'Replaced'} ${count} dialogue line${count===1?'':'s'}.`,'good')}
  ...
  function project(){return {format:'mainvoice-conversation-2',title:$('convTitle').value||'Untitled Conversation',defaultGapMs:+$('convDefaultGap').value||260,masterGainDb:+$('convMasterGain').value||0,compact:!!$('convCompact')?.checked,lines:state.lines.map(l=>({...l}))}}
  function applyProjectObject(p,source=''){if(!['mainvoice-conversation-1','mainvoice-conversation-2'].includes(p.format))throw new Error('That is not a MainVoice conversation project.');stop();$('convTitle').value=p.title||'Untitled Conversation';$('convDefaultGap').value=clamp(+p.defaultGapMs||260,-1200,5000);$('convMasterGain').value=clamp(+p.masterGainDb||0,-18,6);if($('convCompact'))$('convCompact').checked=!!p.compact;state.lines=(Array.isArray(p.lines)?p.lines:[]).map(x=>({id:state.nextId++,voiceId:String(x.voiceId||firstVoiceId()),text:String(x.text||''),expression:EXPRESSIONS[x.expression]?x.expression:'natural',expressionAmount:clamp(+x.expressionAmount||76,0,100),pitch:clamp(+x.pitch||0,-12,12),speed:clamp(+x.speed||1,.55,2.2),gainDb:clamp(+x.gainDb||0,-18,8),pan:clamp(+x.pan||0,-100,100),pauseAfterMs:clamp(Number.isFinite(+x.pauseAfterMs)?+x.pauseAfterMs:260,-1200,5000)}));ensureLineVoices();invalidate();renderLines();msg(`Loaded ${state.lines.length} dialogue line${state.lines.length===1?'':'s'}${source?' from '+source:''}.`,'good')}
  async function loadProject(f){try{applyProjectObject(JSON.parse(await f.text()),f.name)}catch(e){msg(e.message,'bad')}}
  function restoreAutosave(){try{const raw=localStorage.getItem(CONV_AUTOSAVE_KEY);if(!raw){msg('No conversation autosave exists yet.','bad');return}applyProjectObject(JSON.parse(raw),'browser autosave')}catch(e){msg('Could not restore autosave: '+e.message,'bad')}}
  function clearAutosave(){try{localStorage.removeItem(CONV_AUTOSAVE_KEY)}catch{}setAutosaveStatus('Autosave cleared. New edits will create a fresh draft.');msg('Conversation autosave cleared.','good')}

  $('convAddLineBtn').onclick=()=>addLine();$('convEmptyAddBtn').onclick=()=>addLine();$('convAlternateBtn').onclick=alternateVoices;$('convReplaceSpeakerBtn').onclick=()=>replaceSpeaker(false);$('convSwapSpeakersBtn').onclick=()=>replaceSpeaker(true);$('convDemoBtn').onclick=demo;$('convPlayBtn').onclick=playAll;$('convStopBtn').onclick=stop;$('convExportBtn').onclick=exportWav;$('convTranscriptBtn').onclick=exportTranscript;$('convSrtBtn').onclick=exportSrt;$('convRestoreDraftBtn').onclick=restoreAutosave;$('convClearDraftBtn').onclick=clearAutosave;$('convImportScriptBtn').onclick=()=>parseScript(false);$('convAppendScriptBtn').onclick=()=>parseScript(true);
  ...
  $('convDefaultGap').onchange=e=>{e.target.value=clamp(+e.target.value||260,-1200,5000);scheduleAutosave()};$('convMasterGain').onchange=()=>invalidate();$('convTitle').addEventListener('input',scheduleAutosave);$('convCompact').onchange=e=>{try{localStorage.setItem('mainvoice.conversation.compact',e.target.checked?'1':'0')}catch{}renderLines();scheduleAutosave()};
  document.addEventListener('mainvoice:voiceschange',()=>{ensureLineVoices();state.bufferCache.clear();invalidate();renderLines();populateSpeakerTools()});

  // Start empty, but remember compact mode and advertise recoverable autosaves.
  try{$('convCompact').checked=localStorage.getItem('mainvoice.conversation.compact')==='1';const a=JSON.parse(localStorage.getItem(CONV_AUTOSAVE_KEY)||'null');if(a?._autosavedAt)setAutosaveStatus('Autosave available · '+new Date(a._autosavedAt).toLocaleString())}catch{}
  state.autosaveReady=true;populateSpeakerTools();renderLines();
})();
