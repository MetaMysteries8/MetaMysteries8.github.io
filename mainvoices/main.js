(() => {
  ...
  const state = {voices:[],voice:null,buffers:new Map(),voiceBufferCache:new Map(),dict:null,audioCtx:null,currentSource:null,generated:'',active:'',manual:false,unknown:[],rendered:null,autoTokens:[],words:[],emphasis:new Set(),variationSeed:1};
  ...
  function switchTab(name,persist=true){
    const btn=[...document.querySelectorAll('.tab')].find(b=>b.dataset.tab===name);if(!btn)return;
    document.querySelectorAll('.tab').forEach(b=>b.classList.toggle('active',b.dataset.tab===name));document.querySelectorAll('.panel').forEach(p=>p.classList.toggle('active',p.id==='tab-'+name));btn.scrollIntoView?.({block:'nearest',inline:'nearest'});
    if(persist){try{localStorage.setItem('mainvoice.activeTab',name)}catch{}try{history.replaceState(null,'','#'+name)}catch{}}
  }
  function restoreTab(){const valid=new Set([...document.querySelectorAll('.tab')].map(b=>b.dataset.tab));let wanted='';try{wanted=location.hash.replace(/^#/,'')||localStorage.getItem('mainvoice.activeTab')||'synth'}catch{wanted='synth'}switchTab(valid.has(wanted)?wanted:'synth',false)}
  ...
  function encodeAssetPath(path){return String(path||'').replace(/^\/+/, '').split('/').map(encodeURIComponent).join('/')}
  function safeHex(v,fallback=''){const x=String(v||'').trim();return /^#[0-9a-f]{3,8}$/i.test(x)?x:fallback}
  function safeProfileTheme(v){const x=String(v||'default').toLowerCase();return ['default','glass','terminal','sticker','neon','minimal'].includes(x)?x:'default'}
  function safeExternalUrl(v){try{const u=new URL(String(v||''),location.href);return ['http:','https:'].includes(u.protocol)?u.href:''}catch{return''}}
  function mimeForAsset(name){const ext=String(name||'').toLowerCase().split('.').pop();return ext==='svg'?'image/svg+xml':ext==='webp'?'image/webp':ext==='gif'?'image/gif':ext==='jpg'||ext==='jpeg'?'image/jpeg':'image/png'}
  function voiceArtUrl(v){if(!v)return'';if(v.temporary&&v._artUrl)return v._artUrl;if(v.artFile&&v.base_url)return `${v.base_url}/${encodeAssetPath(v.artFile)}`;return''}
  function profileAssetUrl(v,key){if(!v)return'';if(v.temporary&&v._profileAssetUrls?.[key])return v._profileAssetUrls[key];const f=v.profile?.[key];return f&&v.base_url?`${v.base_url}/${encodeAssetPath(f)}`:''}
  function profileStickerUrl(v,index,sticker){if(!v)return'';if(v.temporary&&v._profileAssetUrls?.[`sticker:${index}`])return v._profileAssetUrls[`sticker:${index}`];const f=sticker?.file;return f&&v.base_url?`${v.base_url}/${encodeAssetPath(f)}`:''}
  function revokeTempArt(v){if(!v?.temporary)return;if(v._artUrl){try{URL.revokeObjectURL(v._artUrl)}catch{}v._artUrl=''}for(const u of Object.values(v._profileAssetUrls||{})){try{URL.revokeObjectURL(u)}catch{}}v._profileAssetUrls={}}
  ...
  async function importBrowserPack(file){
    ...
    try{
      ...
      let artUrl='',artFile='';const artCandidates=[meta.artFile,'character.svg','character.png','character.webp','character.jpg','character.jpeg','character.gif','art.svg','art.png'].filter(Boolean).map(x=>String(x).replace(/^\/+/,'') );
      for(const candidate of artCandidates){const key=[...entries.keys()].find(n=>n.toLowerCase()===(prefix+candidate).toLowerCase());if(key){artUrl=URL.createObjectURL(new Blob([entries.get(key)],{type:mimeForAsset(candidate)}));artFile=candidate;break}}
      const profile=meta.profile&&typeof meta.profile==='object'?meta.profile:{},profileUrls={};
      for(const field of ['bannerFile','backgroundFile']){const candidate=String(profile[field]||'').replace(/^\/+/,'');if(!candidate)continue;const key=[...entries.keys()].find(n=>n.toLowerCase()===(prefix+candidate).toLowerCase());if(key)profileUrls[field]=URL.createObjectURL(new Blob([entries.get(key)],{type:mimeForAsset(candidate)}))}
      if(Array.isArray(profile.stickers))profile.stickers.forEach((st,i)=>{const candidate=String(st?.file||'').replace(/^\/+/,'');if(!candidate)return;const key=[...entries.keys()].find(n=>n.toLowerCase()===(prefix+candidate).toLowerCase());if(key)profileUrls[`sticker:${i}`]=URL.createObjectURL(new Blob([entries.get(key)],{type:mimeForAsset(candidate)}))});
      ...
      const id=`browser:${Date.now()}:${baseId}`,v={
        id,name:String(meta.name||'Browser test voice'),author:String(meta.author||''),description:String(meta.description||''),introSentence:String(meta.introSentence||''),icon:String(meta.icon||''),tags:Array.isArray(meta.tags)?meta.tags:[],artFile,profile,engine:meta.engine&&typeof meta.engine==='object'?meta.engine:{},created:meta.created,phonemeSet:meta.phonemeSet||'ARPAbet-39',phonemes:meta.phonemes||P.map(x=>x.c),defaults:meta.defaults||{},sample_count:sampleBytes.size,complete:sampleBytes.size>=39,temporary:true,sourceFile:file.name,_sampleBytes:sampleBytes,_artUrl:artUrl,_profileAssetUrls:profileUrls
      };
  ...
  function renderVoiceOptions(notify=true){
    const sel=$('voiceSelect'),list=$('voiceList'),filter=String($('voiceSearch')?.value||'').trim().toLowerCase();sel.innerHTML='';list.innerHTML='';let shown=0;
    for(const v of state.voices){
      ...
      const tags=voiceTags(v),hay=[v.name,v.author,v.description,v.id,...tags].join(' ').toLowerCase();if(filter&&!hay.includes(filter))continue;shown++;
      const b=document.createElement('button'),art=voiceArtUrl(v),prof=v.profile||{},accent=safeHex(prof.accent,'');b.className='voice-option'+(state.voice?.id===v.id?' active':'')+(art?'':' no-art');if(accent)b.style.setProperty('--voice-accent',accent);const engine=v.engine?.id?` · ${esc(v.engine.id)}`:'';const copy=`<div class="voice-option-copy"><b>${v.icon?`<span class="voice-icon">${esc(v.icon)}</span>`:''}${esc(v.name)}${v.temporary?'<span class="temporary-badge">BROWSER TEST</span>':''}</b><span class="${v.temporary?'temp-line':''}">${esc(v.author||'no author')} · ${v.complete?39:v.sample_count}/39 samples${engine}${v.description?' · '+esc(v.description):''}</span>${tags.length?`<div class="voice-tags">${tags.map(t=>`<span class="voice-tag">${esc(t)}</span>`).join('')}</div>`:''}</div>`;b.innerHTML=art?`<img class="voice-option-thumb" src="${esc(art)}" alt=""><div>${copy}</div>`:copy;b.onclick=()=>loadVoice(v.id);list.appendChild(b);
    }if(filter&&!shown)list.innerHTML='<div class="notice">No voices match that search.</div>';if(notify)try{document.dispatchEvent(new CustomEvent('mainvoice:voiceschange',{detail:{voices:state.voices}}))}catch{}
  }
  ...
  async function getVoiceBuffers(voiceOrId){
    const v=typeof voiceOrId==='string'?state.voices.find(x=>x.id===voiceOrId):voiceOrId;if(!v)throw new Error('That MainVoice is unavailable.');
    const cached=state.voiceBufferCache.get(v.id);if(cached?.size)return cached;
    ...
    const buffers=new Map(pairs.filter(x=>x[1]));state.voiceBufferCache.set(v.id,buffers);return buffers;
  }
  async function loadVoice(id){
    ...
    try{
      state.buffers=await getVoiceBuffers(v);state.voice=v;if(!v.temporary)localStorage.setItem('mainvoice.lastVoice',v.id);$('voiceSelect').value=v.id;applyIntro(v);applyVoiceDefaults(v.defaults||{});renderVoiceInfo();renderVoiceOptions();renderBank();setStatus(`${v.name} ready`);state.rendered=null;try{document.dispatchEvent(new CustomEvent('mainvoice:voicechange',{detail:{voice:v}}))}catch{};
    }catch(e){setStatus('voice load failed','bad');$('voiceInfo').textContent=e.message}finally{document.body.classList.remove('loading')}
  }
  function applyVoiceDefaults(d){if($('synthPresetSelect')){$('synthPresetSelect').value='';$('synthPresetDeleteBtn').disabled=true}if(Number.isFinite(d.speed))$('speed').value=d.speed;if(Number.isFinite(d.pitchSemitones))$('pitch').value=d.pitchSemitones;if(Number.isFinite(d.overlapMs))$('overlap').value=d.overlapMs;if(Number.isFinite(d.wordGapMs))$('wordGap').value=d.wordGapMs;if(Number.isFinite(d.punctuationGapMs))$('punctGap').value=d.punctuationGapMs;['speed','pitch','overlap','wordGap','punctGap','gain'].forEach(id=>$(id).dispatchEvent(new Event('input')));buildPronunciation()}
  function renderVoiceInfo(){
    const v=state.voice;if(!v)return;const tags=voiceTags(v),intro=String(v.introSentence||'').trim(),art=voiceArtUrl(v),prof=v.profile||{};
    const accent=safeHex(prof.accent,'#ff7a18'),accent2=safeHex(prof.accent2,accent),theme=safeProfileTheme(prof.theme),banner=profileAssetUrl(v,'bannerFile'),bg=profileAssetUrl(v,'backgroundFile');
    const badges=(Array.isArray(prof.badges)?prof.badges:[]).slice(0,16).map(x=>typeof x==='string'?{label:x}:{label:String(x?.label||''),icon:String(x?.icon||'')}).filter(x=>x.label);
    const links=(Array.isArray(prof.links)?prof.links:[]).slice(0,12).map(x=>({label:String(x?.label||x?.url||''),url:safeExternalUrl(x?.url)})).filter(x=>x.url&&x.label);
    const engine=v.engine?.id?`<span class="profile-engine-badge">engine: ${esc(v.engine.id)}</span>`:'';
    const identity=`<div class="voice-title">${v.icon?`<span class="voice-icon">${esc(v.icon)}</span>`:''}${esc(v.name)}${v.temporary?'<span class="temporary-badge">BROWSER TEST</span>':''}</div><div class="voice-meta">${v.author?`by ${esc(v.author)} · `:''}${state.buffers.size}/39 samples loaded${v.description?`<br>${esc(v.description)}`:''}${v.temporary?`<br><b>Temporary:</b> loaded from ${esc(v.sourceFile||'a ZIP')} and removed on refresh.`:''}</div>${engine}${tags.length?`<div class="voice-tags">${tags.map(t=>`<span class="voice-tag">${esc(t)}</span>`).join('')}</div>`:''}`;
    const badgeHtml=badges.length?`<div class="profile-badges">${badges.map(b=>`<span class="profile-badge">${b.icon?esc(b.icon)+' ':''}${esc(b.label)}</span>`).join('')}</div>`:'';
    const linkHtml=links.length?`<div class="profile-links">${links.map(l=>`<a href="${esc(l.url)}" target="_blank" rel="noopener noreferrer">${esc(l.label)}</a>`).join('')}</div>`:'';
    const about=(prof.status||prof.bio)?`<div class="profile-about">${prof.status?`<div class="profile-status">● ${esc(prof.status)}</div>`:''}${prof.bio?`<div>${esc(prof.bio)}</div>`:''}</div>`:'';
    const style=`--profile-accent:${accent};--profile-accent2:${accent2};${bg?`--profile-bg:url('${String(bg).replaceAll("'",'%27')}');`:''}`;
    const stickers=(Array.isArray(prof.stickers)?prof.stickers:[]).slice(0,16).map((st,i)=>{const src=profileStickerUrl(v,i,st);if(!src)return'';const x=Math.max(0,Math.min(100,Number(st?.x??82))),y=Math.max(0,Math.min(100,Number(st?.y??12))),size=Math.max(16,Math.min(180,Number(st?.size??52))),rot=Math.max(-180,Math.min(180,Number(st?.rotate??0)));return `<img class="voice-profile-sticker" src="${esc(src)}" alt="" style="left:${x}%;top:${y}%;width:${size}px;transform:translate(-50%,-50%) rotate(${rot}deg)">`}).join('');
    $('voiceInfo').className='';$('voiceInfo').innerHTML=`<div class="voice-profile voice-profile-${theme}" style="${esc(style)}">${banner?`<img class="voice-profile-banner" src="${esc(banner)}" alt="">`:''}${stickers}<div class="voice-profile-body">${art?`<img class="voice-character-art" src="${esc(art)}" alt="Character art for ${esc(v.name)}">`:''}<div class="voice-character-copy">${identity}${about}${badgeHtml}${linkHtml}${intro?`<div class="divider"></div><div class="small"><b>Intro:</b> ${esc(intro)}</div><button id="reloadIntroBtn" class="button secondary" style="margin-top:9px">↻ Put intro in text box</button>`:''}</div></div></div>`;
  ...
  // QoL v5: reusable whole-synth setups. Expression presets alone only change the
  // prosody model; these also remember speed, pitch, joins, pauses, and toggles.
  const SYNTH_PRESET_KEY='mainvoice.synthPresets.v1';
  const BUILTIN_SYNTH_PRESETS={
    'builtin:natural':{name:'Natural default',speed:1,pitch:0,overlap:24,wordGap:45,punctGap:260,gainDb:0,expression:'natural',expressionAmount:72,intonation:100,rhythm:100,humanize:18,autoStress:true,smartOverlap:true,autoPunctuation:true},
    'builtin:raw':{name:'Raw / almost untouched',speed:1,pitch:0,overlap:12,wordGap:42,punctGap:240,gainDb:0,expression:'neutral',expressionAmount:0,intonation:0,rhythm:0,humanize:0,autoStress:false,smartOverlap:true,autoPunctuation:false},
    'builtin:animated':{name:'Animated',speed:1.08,pitch:1,overlap:23,wordGap:38,punctGap:220,gainDb:0,expression:'excited',expressionAmount:92,intonation:150,rhythm:120,humanize:16,autoStress:true,smartOverlap:true,autoPunctuation:true},
    'builtin:story':{name:'Slow storyteller',speed:.84,pitch:-1,overlap:27,wordGap:58,punctGap:360,gainDb:0,expression:'storyteller',expressionAmount:84,intonation:115,rhythm:138,humanize:10,autoStress:true,smartOverlap:true,autoPunctuation:true}
  };
  function synthPresetSnapshot(name=''){return {name,speed:+$('speed').value,pitch:+$('pitch').value,overlap:+$('overlap').value,wordGap:+$('wordGap').value,punctGap:+$('punctGap').value,gainDb:+$('gain').value,expression:$('expressionPreset').value,expressionAmount:+$('expressionAmount').value,intonation:+$('intonation').value,rhythm:+$('rhythm').value,humanize:+$('humanize').value,autoStress:$('autoStress').checked,smartOverlap:$('smartOverlap').checked,autoPunctuation:$('autoPunctuation').checked}}
  function userSynthPresets(){try{const v=JSON.parse(localStorage.getItem(SYNTH_PRESET_KEY)||'{}');return v&&typeof v==='object'&&!Array.isArray(v)?v:{}}catch{return {}}}
  function writeUserSynthPresets(v){try{localStorage.setItem(SYNTH_PRESET_KEY,JSON.stringify(v))}catch{}}
  function renderSynthPresets(selected=''){const sel=$('synthPresetSelect');if(!sel)return;const users=userSynthPresets();sel.innerHTML='<option value="">Choose a synth setup…</option><optgroup label="Built in">'+Object.entries(BUILTIN_SYNTH_PRESETS).map(([id,p])=>`<option value="${id}">${esc(p.name)}</option>`).join('')+'</optgroup>';const names=Object.keys(users).sort((a,b)=>a.localeCompare(b));if(names.length)sel.insertAdjacentHTML('beforeend','<optgroup label="Your presets">'+names.map(n=>`<option value="user:${esc(n)}">${esc(n)}</option>`).join('')+'</optgroup>');if(selected&&[...sel.options].some(o=>o.value===selected))sel.value=selected;$('synthPresetDeleteBtn').disabled=!sel.value.startsWith('user:')}
  function resolveSynthPreset(id){if(BUILTIN_SYNTH_PRESETS[id])return BUILTIN_SYNTH_PRESETS[id];if(id.startsWith('user:'))return userSynthPresets()[id.slice(5)]||null;return null}
  function applySynthPreset(p){if(!p)return;const vals={speed:p.speed,pitch:p.pitch,overlap:p.overlap,wordGap:p.wordGap,punctGap:p.punctGap,gain:p.gainDb,expressionAmount:p.expressionAmount,intonation:p.intonation,rhythm:p.rhythm,humanize:p.humanize};for(const [id,v] of Object.entries(vals)){if(Number.isFinite(+v))$(id).value=v}if(p.expression&&EXPRESSIONS[p.expression])$('expressionPreset').value=p.expression;for(const [id,k] of [['autoStress','autoStress'],['smartOverlap','smartOverlap'],['autoPunctuation','autoPunctuation']])if(typeof p[k]==='boolean')$(id).checked=p[k];for(const id of ['speed','pitch','overlap','wordGap','punctGap','gain','expressionAmount','intonation','rhythm','humanize'])$(id).dispatchEvent(new Event('input'));$('expressionPreset').dispatchEvent(new Event('input'));for(const id of ['autoStress','smartOverlap','autoPunctuation'])$(id).dispatchEvent(new Event('input'));state.rendered=null;$('synthMessage').textContent=`Applied synth setup: ${p.name||'saved preset'}.`;}
  function saveSynthPreset(){const raw=prompt('Name this synth setup:','My setup');if(!raw?.trim())return;const name=raw.trim().slice(0,60),users=userSynthPresets();users[name]=synthPresetSnapshot(name);writeUserSynthPresets(users);renderSynthPresets('user:'+name);$('synthMessage').textContent=`Saved “${name}” in this browser.`;}
  function deleteSynthPreset(){const sel=$('synthPresetSelect'),id=sel?.value||'';if(!id.startsWith('user:'))return;const name=id.slice(5);if(!confirm(`Delete synth setup “${name}”?`))return;const users=userSynthPresets();delete users[name];writeUserSynthPresets(users);renderSynthPresets();$('synthMessage').textContent=`Deleted “${name}”.`;}
  ...
  $('synthPresetSelect').onchange=e=>{$('synthPresetDeleteBtn').disabled=!e.target.value.startsWith('user:')};$('synthPresetApplyBtn').onclick=()=>{const p=resolveSynthPreset($('synthPresetSelect').value);if(p)applySynthPreset(p);else $('synthMessage').textContent='Choose a synth setup first.'};$('synthPresetSaveBtn').onclick=saveSynthPreset;$('synthPresetDeleteBtn').onclick=deleteSynthPreset;renderSynthPresets();
