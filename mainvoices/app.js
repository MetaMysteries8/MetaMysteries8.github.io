...
const state = {
  ...
  stageView: "orb",
  threads: [],            // {id,title,updated,messageCount} index of saved conversations (IndexedDB is the source of truth)
  threadId: localStorage.getItem("thread_id") || "",
  canvasOpen: localStorage.getItem("v3_canvas") !== "0", // V3 right-hand artifact canvas
  canvasView: localStorage.getItem("v3_canvas_view") === "grid" ? "grid" : "tabs",
  canvasIndex: 0,
  railOpen: localStorage.getItem("v3_rail") !== "0",
  ...
  // "1" classic, "2" web-OS desktop, "3" the current shell (rail + conversation +
  // canvas). V3 is the default for anyone who has not explicitly chosen otherwise.
  version: ["1", "2", "3"].includes(localStorage.getItem("ui_version")) ? localStorage.getItem("ui_version") : "3",
...
const SOUND_FILES = {
  ...
  // The V3 intro theme: 17.1s, with the climax at 13.75s and a long resolve out to
  // ~16.2s. playV3Intro is choreographed to that envelope.
  v3Intro: "sounds/11_V3.ogg",
};

// Beat map of sounds/11_V3.ogg (measured RMS envelope), so the intro animation and
// the music hit together instead of drifting apart.
const V3_INTRO = {
  markIn: 0,        // mark forms out of the dark
  structure: 3500,  // the shell's skeleton draws in
  populate: 8250,   // accent — content fills the panes
  surge: 10500,     // build begins
  climax: 13750,    // loudest moment: the flash, real UI swapped in behind it
  settle: 14100,    // overlay starts dissolving
  end: 16200,       // afterglow gone
};
...
// Like playSound, but returns a handle so a long cue (the 17s V3 intro) can be
// faded out when the user skips it instead of playing on over the app.
function playCue(key) {
  const { ctx, master } = sound;
  const buffer = sound.buffers[key];
  if (!ctx || !master || !buffer || sound.muted) return { stop() {} };
  if (ctx.state === "suspended") ctx.resume().catch(() => {});
  const node = ctx.createBufferSource();
  ...
  return {
    stop(fade = 0.45) {
      try {
        ...
        node.stop(ctx.currentTime + fade + 0.02);
...
function init() {
  ...
  startVizLoop();
  bindV3();
  // A conversation carried over from before threads existed becomes thread #1, so
  // nobody loses their history on upgrade.
  if (!state.threadId) { state.threadId = crypto.randomUUID(); localStorage.setItem("thread_id", state.threadId); }
  loadThreads().then(() => { if (state.messages.length) syncActiveThread(); refreshV3Chrome(); });
...
function bindEvents() {
  ...
  el.versionBtn.addEventListener("click", () => setUiVersion(nextUiVersion(), { animate: true }));
  document.querySelector(".account-strip")?.prepend(el.versionBtn);
  updateVersionButton();
  buildOsChrome();
  setInterval(refreshOsChrome, 1000);
  renderDesktopStatus();
  // Keep native v2 windows on-screen when the viewport changes (debounced re-flow).
  window.addEventListener("resize", () => { clearTimeout(veResizeTimer); veResizeTimer = setTimeout(clampWindowsToViewport, 160); });
...
async function handleMainAction() {
  if (!requireKey()) return;
  // Re-entrancy guard: a start is async (mic-permission prompt, socket connecting) and the
  // session flags (state.realtime/gibber/mediaRecorder) aren't set until it resolves. Without
  // this, spam-clicking Start before permission is granted spins up a second getUserMedia +
  // AudioContext + WebSocket each click. Stop actions are synchronous and always allowed.
  if (state.starting) return;
  if (state.mode === "realtime") {
    if (state.realtime) { stopRealtime(); return; }
    state.starting = true;
    try {
      await startRealtime();
    } catch (error) {
  ...
  if (state.mode === "gibber") {
    if (state.gibber) { stopGibberlink(); return; }
    state.starting = true;
    try {
  ...
  if (state.mediaRecorder?.state === "recording") { stopPushRecording(); return; }
  state.starting = true;
  try {
    await startPushRecording();
  } finally {
    state.starting = false;
  }
}
...
function toolDefinitions() {
  const tools = [
    ...
    {
      name: "set_version",
      description: "Switch the whole app UI between its three layouts. \"3\" is the default shell: conversation history in a left rail, the conversation in the centre, and an artifact canvas beside it that tabs or tiles. \"2\" is \"VoiceEnable OS\", a desktop where every artifact becomes its own movable window and the interface can morph. \"1\" is the classic layout: one big orb with a single paged canvas. Call this when the user asks for a different version, the web OS, the classic look, or to go back. Switching to V3 or v2 plays a cinematic reveal; every switch is reversible.",
      parameters: { type: "object", properties: { version: { type: "string", enum: ["1", "2", "3"], description: "Target UI version" } }, required: ["version"] },
    },
    {
      name: "morph_ui",
...
function saveConversation() {
  // localStorage keeps the last 60 for a fast cold start; the active thread in
  // IndexedDB keeps the conversation whole.
...
function renderMessages() {
  ...
  for (const target of targets) {
    target.innerHTML = "";
    // V3 fills an empty conversation with a greeting + starters rather than a void.
    if (!state.messages.length && state.version === "3" && target === el.inlineTranscript) {
      renderV3Hello(target);
      continue;
    }
...
// Artifact payloads arrive in whatever shape the model felt like: an array of rows,
// a {label: value} map, or a JSON string of either. Preserve all three and let
// normalizeRows do the interpreting at render time.
function normalizeArtifactData(data) {
  if (Array.isArray(data)) return data.slice(0, 12);
  if (data && typeof data === "object") return data;
  if (typeof data === "string" && data.trim()) {
    try {
      const parsed = JSON.parse(data);
      return Array.isArray(parsed) ? parsed.slice(0, 12) : (parsed && typeof parsed === "object" ? parsed : []);
    } catch { return []; }
  }
  return [];
}
...
function showWorkspace(args, toolId) {
  ...
  const artifact = {
    ...
    // Keep plain objects as well as arrays: metrics/chart layouts are naturally
    // {label: value} maps, and normalizeRows already understands both. Coercing
    // everything non-array to [] silently emptied those cards.
    data: isWidget ? (args.data ?? null) : normalizeArtifactData(args.data),
  ...
  state.workspaceIndex = 0; // show the freshly added item (v1 pager)
  state.canvasIndex = 0;    // …and the freshly added tab (V3 canvas)
  saveWorkspace();
  renderWorkspace();
...
function renderWorkspace() {
  ...
  if (state.version === "3") { renderWorkspaceV3(); return; }
...
function setUiVersion(version, opts = {}) {
  const target = ["1", "2", "3"].includes(String(version)) ? String(version) : "3";
  ...
  const finish = () => {
    ...
    // and v2 can never inherit a collapsed pane.
    document.body.classList.toggle("rail-hidden", target === "3" && !state.railOpen);
    document.body.classList.toggle("rail-open", target === "3" && state.railOpen);
    document.body.classList.toggle("canvas-closed", target === "3" && !state.canvasOpen);
    document.body.classList.toggle("canvas-open", target === "3" && state.canvasOpen);
    // The inline chat is a toggled view in v1 but the whole point of V3.
    if (target === "3") setStageView("chat");
    renderMessages();
    renderWorkspace();
    updateVersionButton();
    refreshOsChrome();
  ...
  if (opts.animate && toV3) playV3Intro(finish);
  else if (opts.animate && toV2) playV2Transition(finish);
...
// The pill cycles 3 → 1 → 2 → 3, so every layout is reachable without the palette.
function updateVersionButton() {
  if (!el.versionBtn) return;
  const labels = { 1: "✨ v2", 2: "◈ V3", 3: "↩ v1" };
  const titles = {
    1: "Switch to the v2 desktop (VoiceEnable OS)",
    2: "Switch to V3 — rail, conversation and canvas",
    3: "Switch to the classic v1 layout",
  };
  el.versionBtn.textContent = labels[state.version] || "◈ V3";
  el.versionBtn.title = titles[state.version] || titles[3];
  el.versionBtn.setAttribute("aria-pressed", state.version === "3" ? "true" : "false");
}

function nextUiVersion() {
  return state.version === "3" ? "1" : state.version === "1" ? "2" : "3";
}
...
// --- Native window manager (dependency-free, ships in the .exe, works offline). This is
// the single window path in v2. Windows are absolutely-positioned inside the desktop, which
// is inset to the free area (below the top bar, above the taskbar) so they can never hide
// under the OS chrome. See makeWindow / enableWindowDrag / enableWindowResize below. ---
...
// Free desktop area = viewport minus the reserved top bar + taskbar. Used to reflow native
// windows back on-screen after a viewport resize.
function osFreeArea() {
...
// After a viewport resize, pull every native window's stored geometry back inside the
// desktop's free area so nothing can end up off-screen, then re-render.
function clampWindowsToViewport() {
  ...
  state.workspace.forEach((a) => {
    ...
    if (!s || s.max) return;
    const w = Math.min(s.w || 420, availW - 12);
    const h = Math.min(s.h || 360, availH - 12);
    const nx = Math.max(0, Math.min(s.x || 0, availW - Math.min(w, 120)));
    const ny = Math.max(0, Math.min(s.y || 0, availH - 36));
    if (nx !== s.x || ny !== s.y) { s.x = nx; s.y = ny; touched = true; }
  });
  if (touched) { saveWindows(); renderWorkspace(); }
}
...
// Live widgets and coder projects need more room than a note/metric card.
function defaultWinWidth(layout) {
  if (layout === "widget" || layout === "project") return 580;
  if (layout === "image_request" || layout === "image" || layout === "chart" || layout === "table") return 480;
  return 420;
}
...
function makeWindow(artifact, idx) {
  ...
  if (!st.max) {
    ...
    win.style.width = `${st.w || defaultWinWidth(artifact.layout)}px`;
...
function enableWindowResize(win, grip, id) {
  grip.addEventListener("pointerdown", (e) => {
    ...
    const move = (ev) => {
      // Cap the new size to the space left of the desktop's right/bottom edge so a window
      // can never be resized off-screen (the desktop is inset to the free area).
      const maxW = Math.max(240, host.clientWidth - win.offsetLeft - 4);
      const maxH = Math.max(160, host.clientHeight - win.offsetTop - 4);
      win.style.width = `${Math.min(Math.max(240, ow + (ev.clientX - sx)), maxW)}px`;
      win.style.height = `${Math.min(Math.max(160, oh + (ev.clientY - sy)), maxH)}px`;
...
function enableWindowDrag(win, handle, id) {
  handle.addEventListener("pointerdown", (e) => {
    ...
    const move = (ev) => {
      ...
      const maxX = Math.max(0, host.width - win.offsetWidth);
      const maxY = Math.max(0, host.height - win.offsetHeight);
      const nx = Math.max(0, Math.min(ox + (ev.clientX - startX), maxX));
...
function setVersionTool(args = {}) {
  ...
  const notes = {
    ...
    2: "Switched to VoiceEnable OS (v2): every artifact is a movable desktop window and the whole shell can morph. Reversible any time.",
    1: "Switched to the classic v1 layout: the big orb with a single paged canvas.",
  };
  return { ok: true, version: v, note: notes[v] };
}
...
function renderVizFrame() {
  ...
  // The v2 assistant puck and the V3 voice-mode visualizer ride the same loop.
  drawOsViz();
  drawVoiceViz();
}
...
function openDb() {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open("voiceenable-gallery", 3);
    request.onupgradeneeded = () => {
      const db = request.result;
      if (!db.objectStoreNames.contains("items")) db.createObjectStore("items", { keyPath: "id" });
      if (!db.objectStoreNames.contains("widgets")) db.createObjectStore("widgets", { keyPath: "id" });
      // v3: conversation threads. localStorage only ever held ONE conversation and
      // capped it at 60 messages; threads live here so history survives in full.
...
// ===========================================================================
// VERSION 3 — the current shell.
//
// v1 spent half its width on an empty orb column and hid the workspace behind a
// one-at-a-time pager; v2 gave the artifacts room but demoted the conversation to
// a puck in the corner and made the user manage overlapping windows. V3 keeps what
// was familiar (one page, the orb, the same tools) and fixes both: a persistent
// rail with real conversation history, the conversation always in the centre, and
// an artifact canvas beside it that tabs or tiles instead of paging or overlapping.
//
// Structurally this is a RELAYOUT, not a rewrite: #inlineChat, #adaptiveWorkspace,
// .controls and the drawers are v1's own nodes, moved by CSS grid. Every existing
// render function keeps working against one set of elements.
// ===========================================================================

const V3 = {
  paletteIndex: 0,     // highlighted row in the command palette
  paletteRows: [],     // flattened rows currently rendered, for keyboard nav
  voiceOpen: false,
  titling: new Set(),  // thread ids currently being auto-titled (avoids double calls)
};

// ---- Toasts. Errors used to land as a "system" message in the transcript, where
// they interrupted the conversation; transient feedback belongs out of band. ----
function toast(text, kind = "info", ms = 4200) {
  ...
  glyph.textContent = kind === "err" ? "✕" : kind === "ok" ? "✓" : "•";
...
// ===========================================================================
// THREADS — many named conversations, stored whole in IndexedDB.
// state.messages stays the live conversation so nothing else has to change; it is
// mirrored into the active thread record on every save.
// ===========================================================================

async function threadStore(mode = "readonly") {
  const db = await openDb();
  return db.transaction("threads", mode).objectStore("threads");
}
...
async function loadThreads() {
  try {
    ...
    state.threads = rows
      .map((t) => ({ id: t.id, title: t.title, updated: t.updated || 0, messageCount: (t.messages || []).length }))
...
async function putThread(record) {
  try {
    const store = await threadStore("readwrite");
    await txDone(store.put(record));
  } catch { /* quota or private mode — the live conversation still works */ }
}

// Persist the live conversation into the active thread. Called from saveConversation
// so every existing addMessage/updateToolEvent path keeps threads in sync for free.
async function syncActiveThread() {
  if (!state.threadId) return;
  const existing = await getThread(state.threadId);
  const record = {
    id: state.threadId,
  ...
  const entry = state.threads.find((t) => t.id === state.threadId);
  if (entry) { entry.updated = record.updated; entry.messageCount = state.messages.length; }
  else state.threads.unshift({ id: record.id, title: record.title, updated: record.updated, messageCount: state.messages.length });
  state.threads.sort((a, b) => b.updated - a.updated);
  renderThreadList();
  maybeAutoTitle(record);
}

// Name a thread from its first exchange, so history is scannable without the user
// naming anything. Uses the cheapest capable model and falls back to a text slice.
async function maybeAutoTitle(record) {
  if (record.title || V3.titling.has(record.id)) return;
  const firstUser = state.messages.find((m) => m.role === "user" && String(m.content || "").trim());
  if (!firstUser) return;
  V3.titling.add(record.id);
  const fallback = shortPrompt(String(firstUser.content)).slice(0, 46);
  let title = fallback;
  if (state.apiKey) {
    const result = await postJson("/v1/chat/completions", {
      model: value("textModel") || "openai",
      messages: [
        { role: "system", content: "Title this conversation in 2-5 words. Reply with the title only — no quotes, no punctuation at the end." },
        { role: "user", content: String(firstUser.content).slice(0, 500) },
      ],
      max_tokens: 20,
    }).catch(() => null);
    const answer = result?.choices?.[0]?.message?.content;
    if (answer) title = answer.trim().replace(/^["'\s]+|["'\s.]+$/g, "").slice(0, 60) || fallback;
  }
  const fresh = await getThread(record.id);
  if (!fresh || fresh.title) { V3.titling.delete(record.id); return; }
  fresh.title = title;
  await putThread(fresh);
  const entry = state.threads.find((t) => t.id === record.id);
  if (entry) entry.title = title;
  V3.titling.delete(record.id);
  renderThreadList();
  refreshV3Chrome();
}

async function newThread(opts = {}) {
  // Nothing typed yet? Reuse the empty thread instead of stacking blank ones up.
  if (!state.messages.length && state.threadId) { if (!opts.silent) toast("Already on a new chat"); return; }
  state.threadId = crypto.randomUUID();
  localStorage.setItem("thread_id", state.threadId);
  state.messages = [];
  saveConversation();
  renderMessages();
  renderThreadList();
  ...
  if (!opts.silent) {
    playSound("connect");
    document.querySelector("#inlineTextInput")?.focus();
  }
}

async function openThread(id) {
  if (id === state.threadId) return;
  const record = await getThread(id);
  if (!record) { toast("That conversation is no longer stored", "err"); loadThreads(); return; }
  state.threadId = id;
...
async function deleteThread(id) {
  try {
    const store = await threadStore("readwrite");
    await txDone(store.delete(id));
  } catch { /* ignore */ }
  state.threads = state.threads.filter((t) => t.id !== id);
  if (id === state.threadId) { state.threadId = ""; state.messages = []; saveConversation(); renderMessages(); }
  renderThreadList();
  refreshV3Chrome();
...
function threadTitle(entry) {
  return entry?.title || (entry?.messageCount ? "Untitled chat" : "New chat");
}

// Group history the way every mail/chat client does: recency buckets, not a flat list.
function threadBucket(updated) {
  const day = 86400000;
  const now = Date.now();
  const startOfToday = new Date().setHours(0, 0, 0, 0);
  if (updated >= startOfToday) return "Today";
  if (updated >= startOfToday - day) return "Yesterday";
  if (updated >= now - day * 7) return "Previous 7 days";
  if (updated >= now - day * 30) return "Previous 30 days";
  return "Older";
}

function renderThreadList() {
  const host = document.querySelector("#v3Threads");
  if (!host) return;
  host.innerHTML = "";
  if (!state.threads.length) {
    const empty = document.createElement("p");
    empty.className = "v3-threads-empty";
    empty.textContent = "Your conversations will appear here. Each one keeps its own history.";
    host.append(empty);
    return;
  }
  let bucket = "";
  state.threads.forEach((entry) => {
    const group = threadBucket(entry.updated);
    if (group !== bucket) {
      bucket = group;
      const head = document.createElement("div");
      head.className = "v3-thread-group";
      head.textContent = group;
      host.append(head);
    }
    const row = document.createElement("button");
    row.type = "button";
    row.className = `v3-thread${entry.id === state.threadId ? " active" : ""}`;
    const name = document.createElement("span");
    name.className = "v3-thread-name";
    name.textContent = threadTitle(entry);
    const kill = document.createElement("span");
    kill.className = "v3-thread-kill";
    kill.textContent = "✕";
    ...
    row.title = `${threadTitle(entry)} · ${entry.messageCount} message${entry.messageCount === 1 ? "" : "s"}`;
    row.addEventListener("click", () => openThread(entry.id));
    host.append(row);
  });
}

// ===========================================================================
// V3 CHROME — live bits of the rail and top bar.
// ===========================================================================
function refreshV3Chrome() {
  if (state.version !== "3") return;
  const entry = state.threads.find((t) => t.id === state.threadId);
  const title = document.querySelector("#v3ThreadTitle");
  if (title) title.textContent = threadTitle(entry);
  const meta = document.querySelector("#v3ThreadMeta");
  if (meta) {
    const n = state.messages.filter((m) => m.role === "user" || m.role === "agent").length;
    meta.textContent = n ? `${n} message${n === 1 ? "" : "s"} · ${state.mode}` : state.mode;
  }
  const info = modelInfo(state.mode === "realtime" ? realtimeModel() : value("textModel"));
  const name = document.querySelector("#v3ModelName");
  if (name) name.textContent = info.title || info.name || "—";
  const pill = document.querySelector("#v3ModelPill");
  if (pill) pill.title = `${state.mode === "realtime" ? "Realtime" : "Assistant"} model: ${info.name}${info.desc ? `\n${info.desc}` : ""}\nClick to change`;
  const dot = document.querySelector("#v3AuthDot");
  const label = document.querySelector("#v3AuthLabel");
  if (dot) dot.classList.toggle("ok", !!state.apiKey);
  if (label) label.textContent = state.apiKey ? "Connected" : "Not connected";
  const pollen = document.querySelector("#v3Pollen");
  if (pollen && el.pollenBalance) pollen.textContent = el.pollenBalance.textContent;
  const sfx = document.querySelector("#v3SoundToggle");
  if (sfx) sfx.textContent = sound.muted ? "🔇" : "🔊";
  const canvasBtn = document.querySelector("#v3CanvasToggle");
  if (canvasBtn) canvasBtn.setAttribute("aria-pressed", state.canvasOpen ? "true" : "false");
  const mic = document.querySelector("#v3Mic");
  if (mic) mic.classList.toggle("live", !!state.realtime || !!state.mediaRecorder);
}

function setRailOpen(open) {
  state.railOpen = !!open;
  localStorage.setItem("v3_rail", state.railOpen ? "1" : "0");
  document.body.classList.toggle("rail-hidden", !state.railOpen);
  document.body.classList.toggle("rail-open", state.railOpen);
}

function setCanvasOpen(open) {
  state.canvasOpen = !!open;
  localStorage.setItem("v3_canvas", state.canvasOpen ? "1" : "0");
  document.body.classList.toggle("canvas-closed", !state.canvasOpen);
  document.body.classList.toggle("canvas-open", state.canvasOpen);
  refreshV3Chrome();
}

// ===========================================================================
// V3 CANVAS — tabs (one artifact, full height) or grid (all of them, tiled).
// Replaces v1's 1-of-N pager and v2's overlapping windows.
// ===========================================================================
function renderWorkspaceV3() {
  const host = el.adaptiveWorkspace;
  host.classList.remove("ve-desktop", "paged");
  host.innerHTML = "";
  const items = state.workspace;
  el.workspaceMode.textContent = items.length ? `Canvas · ${items.length}` : "Canvas";
  // The canvas auto-opens the moment the agent puts something on it.
  if (items.length && !state.canvasOpen) setCanvasOpen(true);
  renderCanvasTabs();
  if (!items.length) {
    const empty = document.createElement("div");
    empty.className = "v3-canvas-empty";
    const glyph = document.createElement("div");
    glyph.className = "glyph";
    glyph.textContent = "▥";
    const strong = document.createElement("strong");
    strong.textContent = "Nothing on the canvas yet";
    const p = document.createElement("p");
    p.textContent = "Charts, tables, widgets, images and generated media land here while you keep talking.";
    empty.append(glyph, strong, p);
    host.append(empty);
    return;
  }
  state.canvasIndex = Math.max(0, Math.min(state.canvasIndex, items.length - 1));
  const scroll = document.createElement("div");
  scroll.className = "v3-canvas-scroll";
  if (state.canvasView === "grid") {
    const grid = document.createElement("div");
    grid.className = "v3-canvas-grid";
    items.forEach((artifact) => grid.append(renderArtifact(artifact)));
    scroll.append(grid);
  } else {
    scroll.append(renderArtifact(items[state.canvasIndex]));
  }
  host.append(scroll);
}

function renderCanvasTabs() {
  const bar = document.querySelector("#v3CanvasTabs");
  if (!bar) return;
  bar.innerHTML = "";
  const grid = state.canvasView === "grid";
  state.workspace.forEach((artifact, index) => {
    const tab = document.createElement("button");
    tab.type = "button";
    tab.className = `v3-canvas-tab${!grid && index === state.canvasIndex ? " active" : ""}`;
    tab.setAttribute("role", "tab");
    const glyph = document.createElement("span");
    glyph.textContent = windowGlyph(artifact.layout);
    const label = document.createElement("span");
    label.textContent = artifact.title || artifact.layout;
    tab.append(glyph, label);
    tab.title = artifact.title || artifact.layout;
    tab.addEventListener("click", () => {
      state.canvasIndex = index;
      if (state.canvasView === "grid") setCanvasView("tabs");
      else renderWorkspace();
    });
    bar.append(tab);
  });
  const gridBtn = document.querySelector("#v3CanvasGrid");
  if (gridBtn) {
    gridBtn.setAttribute("aria-pressed", grid ? "true" : "false");
    gridBtn.title = grid ? "Show one at a time" : "Tile everything";
  }
}

function setCanvasView(view) {
  state.canvasView = view === "grid" ? "grid" : "tabs";
  localStorage.setItem("v3_canvas_view", state.canvasView);
  renderWorkspace();
}

// ===========================================================================
// VOICE MODE — the orb stops owning a column. It sits in the composer and expands
// to a full-screen listening view while a session is live.
// ===========================================================================
function openVoiceMode() {
  ...
  document.querySelector("#voiceMode").textContent = state.mode === "realtime" ? "Realtime" : state.mode === "push" ? "Push to talk" : "Gibberlink";
  syncVoiceCaption();
}
...
// Show the tail of the conversation as a live caption, so a voice session is
// followable without reading the transcript behind the overlay.
function syncVoiceCaption() {
  const caption = document.querySelector("#voiceCaption");
  if (!caption || !V3.voiceOpen) return;
  const last = [...state.messages].reverse().find((m) => m.role === "agent" || m.role === "user");
  caption.textContent = last ? String(last.content || "").slice(0, 320) : "";
  const status = document.querySelector("#voiceStatus");
  if (status && el.realtimeStatus) status.textContent = el.realtimeStatus.textContent;
}

// Full-bleed version of the reactive visualizer, fed by the same analyser/level the
// orb and the v2 puck use.
function drawVoiceViz() {
  const cv = document.querySelector("#voiceViz");
  if (!cv || !V3.voiceOpen || document.hidden) return;
  const ctx = cv.getContext("2d");
  const W = cv.width;
  const H = cv.height;
  const cx = W / 2;
  ...
  // Three concentric reactive rings + a breathing core.
  const rings = [
    { r: 118, n: 84, w: 2.2, a: 0.5, c: "#4de8ff" },
    { r: 150, n: 108, w: 1.8, a: 0.34, c: "#8e6cff" },
    { r: 184, n: 132, w: 1.4, a: 0.22, c: "#ff4fd8" },
  ];
  ctx.lineCap = "round";
  rings.forEach((ring, ri) => {
    for (let i = 0; i < ring.n; i += 1) {
      const a = (i / ring.n) * Math.PI * 2 - Math.PI / 2 + viz.spin * (0.2 + ri * 0.1);
      let mag;
      if (freq) { const idx = Math.floor((i / ring.n) * freq.length * 0.6); mag = freq[idx] / 255; }
      else mag = 0.16 + 0.55 * level * (0.5 + 0.5 * Math.sin(viz.phase * 2 + i * 0.35 + ri));
      const len = 5 + mag * (30 + ri * 8) + burst * 16;
      const x0 = cx + Math.cos(a) * ring.r;
      const y0 = cy + Math.sin(a) * ring.r;
      ctx.strokeStyle = ring.c;
      ctx.globalAlpha = ring.a * (0.4 + 0.7 * mag);
  ...
  ctx.globalAlpha = 1;
  const coreR = 66 + level * 34 + burst * 20;
  const grad = ctx.createRadialGradient(cx, cy, 0, cx, cy, coreR);
  grad.addColorStop(0, "rgba(255,255,255,0.96)");
  grad.addColorStop(0.42, "#bfeaff");
  grad.addColorStop(0.72, "#8e6cff");
  grad.addColorStop(1, "rgba(0,0,0,0)");
  ctx.fillStyle = grad;
  ctx.beginPath();
  ctx.arc(cx, cy, coreR, 0, Math.PI * 2);
...
// ===========================================================================
// COMMAND PALETTE — one search over threads, models, settings, gallery, widgets
// and actions. 320 models and ~30 settings are otherwise several clicks deep.
// ===========================================================================
...
// Every addressable thing in the app, as flat rows the palette can filter.
function paletteSources() {
  ...
  groups.push({
    label: "Actions",
    rows: [
      { glyph: "✎", title: "New chat", hint: "⌘⇧O", run: () => newThread() },
      { glyph: "◉", title: state.realtime ? "Stop the live session" : "Start a live voice session", run: () => (state.realtime ? stopAll() : handleMainAction()) },
      { glyph: "▥", title: state.canvasOpen ? "Hide the canvas" : "Show the canvas", run: () => setCanvasOpen(!state.canvasOpen) },
      { glyph: "⊞", title: state.canvasView === "grid" ? "Canvas: one at a time" : "Canvas: tile everything", run: () => setCanvasView(state.canvasView === "grid" ? "tabs" : "grid") },
      { glyph: "🎨", title: "Open the image generator", run: () => openGenerator("image") },
      { glyph: "🎬", title: "Open the video generator", run: () => openGenerator("video") },
      { glyph: "🎥", title: "Open the movie maker", run: () => openGenerator("movie") },
      { glyph: "🧹", title: "Clear the canvas", run: () => clearWorkspace() },
      { glyph: sound.muted ? "🔇" : "🔊", title: sound.muted ? "Unmute interface sounds" : "Mute interface sounds", run: () => { setSoundMuted(!sound.muted); refreshV3Chrome(); } },
      { glyph: "⌥", title: "Switch to the classic v1 layout", run: () => setUiVersion("1", { animate: true }) },
      { glyph: "⌥", title: "Switch to the v2 desktop (VoiceEnable OS)", run: () => setUiVersion("2", { animate: true }) },
    ],
  });
  ...
  if (state.threads.length) {
    groups.push({
      label: "Conversations",
      rows: state.threads.slice(0, 40).map((t) => ({
        glyph: "◷",
        title: threadTitle(t),
        sub: `${t.messageCount} message${t.messageCount === 1 ? "" : "s"} · ${threadBucket(t.updated)}`,
        run: () => openThread(t.id),
      })),
    });
  }

  // Models: switching the assistant/coder/image model is the single most common
  // deep setting, so make it reachable by name.
  const roles = [
    { id: "textModel", label: "assistant", list: assistantModels() },
    { id: "coderModel", label: "coder", list: coderModels() },
    { id: "imageModel", label: "image", list: imageModelsOnly() },
    { id: "videoModel", label: "video", list: videoModelsOnly() },
  ];
  const modelRows = [];
  roles.forEach((role) => {
    communityFilter(role.list, value(role.id)).slice(0, 60).forEach((info) => {
      modelRows.push({
        glyph: "◈",
        title: `Set ${role.label} model: ${info.title}`,
        sub: [info.brand, info.name, info.paid ? "paid" : "free", info.community ? "community" : ""].filter(Boolean).join(" · "),
        run: () => {
          const select = document.querySelector(`#${role.id}`);
          select.value = info.name;
          select.dispatchEvent(new Event("change", { bubbles: true }));
          toast(`${role.label} model → ${info.title}`, "ok");
        },
      });
    });
  });
  groups.push({ label: "Models", rows: modelRows });

  groups.push({
    label: "Settings",
    rows: [...document.querySelectorAll(".config-pane > label, .config-pane > .field")].map((field) => {
      const name = (field.querySelector(".field-title") || field.firstChild)?.textContent?.trim() || "Setting";
      const pane = field.closest(".config-pane")?.dataset.pane || "models";
      return {
        glyph: "⚙",
        title: name.replace(/\s+/g, " ").slice(0, 60),
        sub: `Settings · ${pane}`,
        run: () => {
          openDrawer("settingsDrawer");
          showConfigTab(pane);
  ...
  if (state.savedWidgetIndex?.length) {
    groups.push({
      label: "Saved widgets",
      rows: state.savedWidgetIndex.map((w) => ({
        glyph: "▧",
        title: w.title || "Widget",
        sub: w.spec ? shortPrompt(w.spec) : "Saved widget",
        run: () => openSavedWidgetTool({ id: w.id }),
      })),
    });
  }
...
function renderPalette() {
  const host = document.querySelector("#paletteResults");
  if (!host) return;
  const query = document.querySelector("#paletteInput").value.trim().toLowerCase();
  const terms = query.split(/\s+/).filter(Boolean);
  const match = (row) => terms.every((t) => `${row.title} ${row.sub || ""}`.toLowerCase().includes(t));
  host.innerHTML = "";
  V3.paletteRows = [];
  let shown = 0;
  paletteSources().forEach((group) => {
    const hits = group.rows.filter(match).slice(0, query ? 12 : 6);
    ...
    host.append(head);
    hits.forEach((row) => {
      const index = V3.paletteRows.length;
      V3.paletteRows.push(row);
      ...
      node.addEventListener("click", () => runPaletteRow(index));
      host.append(node);
      shown += 1;
    });
  });
  ...
  V3.paletteIndex = Math.min(V3.paletteIndex, Math.max(0, V3.paletteRows.length - 1));
  highlightPaletteRow();
}

function highlightPaletteRow() {
  const rows = [...document.querySelectorAll(".palette-row")];
  rows.forEach((row) => row.classList.toggle("active", Number(row.dataset.index) === V3.paletteIndex));
  rows.find((row) => row.classList.contains("active"))?.scrollIntoView({ block: "nearest" });
}

function runPaletteRow(index) {
  const row = V3.paletteRows[index];
  if (!row) return;
  closePalette();
  try { row.run(); } catch (error) { toast(`That action failed: ${error?.message || error}`, "err"); }
}

function palettePickKey(event) {
  if (!paletteIsOpen()) return false;
  if (event.key === "Escape") { closePalette(); return true; }
  if (event.key === "ArrowDown") { V3.paletteIndex = Math.min(V3.paletteIndex + 1, V3.paletteRows.length - 1); highlightPaletteRow(); return true; }
  if (event.key === "ArrowUp") { V3.paletteIndex = Math.max(V3.paletteIndex - 1, 0); highlightPaletteRow(); return true; }
  if (event.key === "Enter") { runPaletteRow(V3.paletteIndex); return true; }
  return false;
}


// ===========================================================================
// V3 INTRO — choreographed to sounds/11_V3.ogg (see V3_INTRO for the beat map).
// Plays in full only when arriving in V3 from another version; a normal reload
// gets nothing, because a 17-second cutscene on every launch is a tax, not a
// feature (which is exactly what v2's boot gate got wrong).
// ===========================================================================
function playV3Intro(done) {
  const overlay = document.createElement("div");
  overlay.id = "v3-intro";
  overlay.innerHTML =
    "<div class='v3i-field'></div>" +
    "<div class='v3i-shell'></div>" +
    "<div class='v3i-rail'></div>" +
    "<div class='v3i-canvas'></div>" +
    "<div class='v3i-mark'></div>" +
    "<div class='v3i-caption'>VoiceEnable V3</div>" +
    "<div class='v3i-flash'></div>" +
    "<button class='v3i-skip' type='button'>Skip</button>";
  document.body.append(overlay);
...
function bindV3() {
  ...
  on("#v3NewThread", "click", () => newThread());
  on("#v3OpenPalette", "click", () => openPalette());
  on("#v3RailToggle", "click", () => setRailOpen(false));
  on("#v3RailShow", "click", () => setRailOpen(true));
  on("#v3Brand", "click", () => setStageView("chat"));
  on("#v3Account", "click", () => (state.apiKey ? openDrawer("settingsDrawer") : connectByop()));
  on("#v3SoundToggle", "click", () => { setSoundMuted(!sound.muted); refreshV3Chrome(); });
  on("#v3CanvasToggle", "click", () => setCanvasOpen(!state.canvasOpen));
  on("#v3CanvasGrid", "click", () => setCanvasView(state.canvasView === "grid" ? "tabs" : "grid"));
  on("#v3CanvasClear", "click", () => clearWorkspace());
  on("#v3VersionBtn", "click", () => setUiVersion(state.version === "3" ? "1" : "3", { animate: true }));
  on("#v3ModelPill", "click", () => {
    openDrawer("settingsDrawer");
    showConfigTab(state.mode === "realtime" ? "voice" : "models");
    togglePicker(state.mode === "realtime" ? "realtimeModel" : "textModel", true);
  });
  document.querySelectorAll("#v3Rail .v3-nav-btn[data-drawer]").forEach((button) => {
    button.addEventListener("click", () => toggleDrawer(button.dataset.drawer));
  });

  // Voice overlay.
  on("#voiceEnd", "click", () => { stopAll(); closeVoiceMode(); });
  on("#voiceMinimize", "click", closeVoiceMode);
  on("#voiceMute", "click", () => {
    const track = state.realtime?.stream?.getAudioTracks?.()[0];
    if (!track) { toast("No microphone in this mode", "err"); return; }
    track.enabled = !track.enabled;
    const button = document.querySelector("#voiceMute");
    button.classList.toggle("muted", !track.enabled);
    button.title = track.enabled ? "Mute microphone" : "Unmute microphone";
  });
  document.querySelector("#voiceOverlay .voice-scrim")?.addEventListener("click", closeVoiceMode);

  // Palette.
  on("#paletteInput", "input", () => { V3.paletteIndex = 0; renderPalette(); });
  document.querySelector("#palette .palette-scrim")?.addEventListener("click", closePalette);

  // A mic button in the composer: the orb's job, without the orb's column.
  const composer = document.querySelector("#inlineTextForm");
  if (composer && !document.querySelector("#v3Mic")) {
    const mic = document.createElement("button");
    mic.id = "v3Mic";
    mic.type = "button";
    mic.className = "button ghost v3-mic";
    mic.textContent = "🎤";
    mic.title = "Voice mode";
    mic.setAttribute("aria-label", "Voice mode");
    mic.addEventListener("click", () => {
      if (state.realtime || state.mediaRecorder) { openVoiceMode(); return; }
      handleMainAction();
      openVoiceMode();
    });
    composer.insertBefore(mic, composer.querySelector(".composer-text"));
  }
  // v1's placeholder is written for a wide bar; it truncates in V3's narrower column.
  const input = document.querySelector("#inlineTextInput");
  if (input && state.version === "3") input.placeholder = "Message VoiceEnable…";

  document.addEventListener("keydown", v3Shortcuts);
}

function v3Shortcuts(event) {
  if (palettePickKey(event)) { event.preventDefault(); return; }
  const mod = event.metaKey || event.ctrlKey;
  if (mod && event.key.toLowerCase() === "k") { event.preventDefault(); openPalette(); return; }
  if (state.version !== "3") return;
  if (mod && event.shiftKey && event.key.toLowerCase() === "o") { event.preventDefault(); newThread(); return; }
  if (mod && event.key === "\\") { event.preventDefault(); setRailOpen(!state.railOpen); return; }
  if (event.key === "Escape" && V3.voiceOpen) { closeVoiceMode(); return; }
  const typing = /^(INPUT|TEXTAREA|SELECT)$/.test(document.activeElement?.tagName || "") || document.activeElement?.isContentEditable;
  if (typing) return;
  if (event.key === "/") { event.preventDefault(); document.querySelector("#inlineTextInput")?.focus(); }
}

// The empty-conversation state: a greeting plus starters that actually exercise the
// app's capabilities, so a first-time user has somewhere to click.
function renderV3Hello(target) {
  const wrap = document.createElement("div");
  wrap.className = "v3-hello";
  const mark = document.createElement("div");
  mark.className = "v3-hello-mark";
  const h2 = document.createElement("h2");
  h2.textContent = state.apiKey ? "What are we making?" : "Welcome to VoiceEnable";
  const p = document.createElement("p");
  p.textContent = state.apiKey
    ? "Talk or type — it works the same either way. Anything visual lands on the canvas beside this conversation."
    : "Connect your Pollen to start. Everything runs on your own key, and generated media stays on this device.";
  wrap.append(mark, h2, p);
  const starters = state.apiKey
    ? [
      "Make me an image of a neon city at dusk",
      "Build a widget to track my workouts",
      "Chart last quarter's spend by channel",
      "Search the web and summarise it",
    ]
    : ["Connect Pollen"];
  const row = document.createElement("div");
  row.className = "v3-suggests";
  starters.forEach((text) => {
    const chip = document.createElement("button");
    chip.type = "button";
    chip.className = "v3-suggest";
    chip.textContent = text;
    chip.addEventListener("click", () => {
      if (!state.apiKey) { connectByop(); return; }
      const input = document.querySelector("#inlineTextInput");
      if (!input) return;
      input.value = text;
      input.focus();
      document.querySelector("#inlineTextForm")?.requestSubmit();
    });
    row.append(chip);
  });
  wrap.append(row);
