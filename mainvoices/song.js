...
(() => {
  ...
  function setDirty(){ song.rendered=null; draw(); drawPitchCurve(); }

  function defaultTrack(){ return {id:1,name:'Lead',gainDb:0,pan:0,mute:false,solo:false,color:TRACK_COLORS[0]}; }
  ...
  function renderTracks(){
    const list=$('songTrackList');list.innerHTML='';for(const t of song.tracks){const row=document.createElement('div');row.className='daw-track-row'+(t.id===song.activeTrack?' active':'');row.dataset.trackId=t.id;
      row.innerHTML=`<button class="daw-track-select" title="Draw new notes on this track"><span class="daw-track-dot" style="background:${esc(t.color)}"></span><span>${esc(t.name)}</span></button><button class="mini-toggle ${t.mute?'active':''}" data-act="mute" title="Mute">M</button><button class="mini-toggle ${t.solo?'active':''}" data-act="solo" title="Solo">S</button><label class="mini-fader" title="Track gain"><span>VOL</span><input data-act="gain" type="range" min="-24" max="6" step="1" value="${t.gainDb}"></label><label class="mini-fader" title="Track pan"><span>PAN</span><input data-act="pan" type="range" min="-100" max="100" step="1" value="${t.pan}"></label>`;
      row.querySelector('.daw-track-select').onclick=()=>{song.activeTrack=t.id;renderTracks();draw();updateInspector(false)};
  ...
  function addTrack(){if(song.tracks.length>=8){msg('Eight vocal tracks is enough chaos for one browser tab 😭',true);return}remember();const id=song.nextTrackId++,t={id,name:`Vocal ${song.tracks.length+1}`,gainDb:0,pan:0,mute:false,solo:false,color:TRACK_COLORS[(id-1)%TRACK_COLORS.length]};song.tracks.push(t);song.activeTrack=id;renderTracks();setDirty();}
  ...
  function basePitchForNote(n,settings){return n.midi-settings.baseMidi+settings.transpose+(n.fineCents||0)/100;}
  ...
  function notePlan(n,settings,startOverride=null){
    ...
    const startBeat=startOverride==null?n.start:0,noteStart=(startOverride==null?n.start*beatSec:startOverride)+timingJitter,noteDur=Math.max(.06,n.duration*beatSec),noteEnd=noteStart+noteDur;
  ...
  async function renderSong(onlyNote=null){
    ...
    const soloed=song.tracks.filter(t=>t.solo).map(t=>t.id),audibleTrack=t=>!t.mute&&(!soloed.length||soloed.includes(t.id));const plans=[];for(const n of notes){const t=trackById(n.trackId);if(onlyNote||(t&&audibleTrack(t)))plans.push(...notePlan(n,settings,onlyNote?.08:null));}
    ...
    for(const e of plans){const source=app.buffers.get(e.code),win=sourceWindow(e.code,source),target=Math.max(.018,e.dur),natural=Math.min(source.duration,win),useSustain=SUSTAINABLE.has(e.code)&&target>natural*1.04;
      if(e.dynamic){let base;if(useSustain){const prof=sustainProfile(e.code,source,win);base=MainVoiceDSP.sustainToAudioBuffer(temp,source,{pitchSemitones:0,targetSeconds:target,maxSourceSeconds:win,...prof});}else{base=MainVoiceDSP.toAudioBuffer(temp,source,{pitchSemitones:0,speed:clamp(win/target,.08,8),maxSourceSeconds:win});}renderEvents.push({...e,buffer:curvePitchBuffer(temp,base,e.pitchAt)});continue;}
      const pitchQ=Math.round(e.pitch*20)/20,durQ=Math.round(target*1000)/1000,key=`${e.code}|${pitchQ}|${durQ}|${useSustain?'hold':'short'}`;let buf=cache.get(key);if(!buf){if(useSustain){const prof=sustainProfile(e.code,source,win);buf=MainVoiceDSP.sustainToAudioBuffer(temp,source,{pitchSemitones:pitchQ,targetSeconds:target,maxSourceSeconds:win,...prof});}else{buf=MainVoiceDSP.toAudioBuffer(temp,source,{pitchSemitones:pitchQ,speed:clamp(win/target,.08,8),maxSourceSeconds:win});}cache.set(key,buf)}renderEvents.push({...e,buffer:buf});
    }
  ...
  function animatePlayhead(){if(!song.source)return;const s=songSettings(),elapsed=(performance.now()-song.playStartMs)/1000;let beat=song.playStartBeat+elapsed/secPerBeat();if(s.loop&&s.loopEnd>s.loopStart&&beat>=s.loopEnd)beat=s.loopStart+((beat-s.loopStart)%(s.loopEnd-s.loopStart));song.playheadBeat=beat;if(!s.loop&&beat>totalBeats()+.5){stopSong();return}draw();song.playRaf=requestAnimationFrame(animatePlayhead);}
  ...
  // ---------- project save/load ----------
  function projectObject(){const s=songSettings();return{format:'mainvoice-song-2',title:$('songTitle').value||'Untitled MainVoice Song',voiceHint:app.voice?.id||'',bpm:s.bpm,bars:+$('songBars').value||8,snap:snap(),baseMidi:s.baseMidi,transpose:s.transpose,legatoMs:s.legato,consonantLeadMs:s.lead,gainDb:s.gainDb,humanize:Math.round(s.humanize*100),zoomX:pxBeat(),zoomY:rowH(),loop:s.loop,loopStart:s.loopStart,loopEnd:s.loopEnd,tracks:song.tracks.map(t=>({...t})),activeTrack:song.activeTrack,notes:sortedNotes().map(n=>({...n})),backing:{name:song.backing.name,gainDb:song.backing.gainDb,offsetBeats:song.backing.offsetBeats,mute:song.backing.mute}};}
  ...
  $('songNoteDuplicateBtn').onclick=duplicateSelected;$('songNoteDeleteBtn').onclick=deleteSelected;$('songCopyBtn').onclick=copySelected;$('songPasteBtn').onclick=pasteClipboard;$('songQuantizeBtn').onclick=quantizeSelected;$('songOctaveUpBtn').onclick=()=>transposeSelected(12);$('songOctaveDownBtn').onclick=()=>transposeSelected(-12);$('songSemiUpBtn').onclick=()=>transposeSelected(1);$('songSemiDownBtn').onclick=()=>transposeSelected(-1);$('songAutoSlidesBtn').onclick=autoSlides;
  $('songApplyLyricsBtn').onclick=applyLyrics;$('songDemoBtn').onclick=demo;$('songClearBtn').onclick=clearNotes;
  $('songAddTrackBtn').onclick=addTrack;$('songRemoveTrackBtn').onclick=removeTrack;$('songRenameTrackBtn').onclick=renameTrack;
  ...
  $('songLoadInput').onchange=async e=>{const f=e.target.files?.[0];if(f)await loadProjectFile(f);e.target.value=''};
  ...
  $('songVoiceSelect').onchange=e=>app.loadVoice(e.target.value);document.addEventListener('mainvoice:voiceschange',populateVoices);document.addEventListener('mainvoice:voicechange',()=>{populateVoices();msg(`Active singer: ${app.voice?.name||'none'}.`)});
  ...
  for(const id of ['songLoop','songLoopStart','songLoopEnd'])$(id).addEventListener('change',()=>{const s=songSettings();if(s.loopEnd<=s.loopStart)$('songLoopEnd').value=Math.min(totalBeats(),s.loopStart+4);draw()});
  ...
  document.addEventListener('keydown',e=>{
    ...
    if(mod&&e.key.toLowerCase()==='z'){e.preventDefault();e.shiftKey?redo():undo();return}if(mod&&e.key.toLowerCase()==='y'){e.preventDefault();redo();return}if(mod&&e.key.toLowerCase()==='c'&&!edit){e.preventDefault();copySelected();return}if(mod&&e.key.toLowerCase()==='v'&&!edit){e.preventDefault();pasteClipboard();return}
  ...
  // ---------- init ----------
  song.tracks=[defaultTrack()];populateNoteSelectors();populateVoices();renderTracks();refreshControlOutputs();setTool('pencil');resizeCanvas();updateInspector();updateUndoButtons();setCursor(0);$('songBackingGain').value=song.backing.gainDb;$('songBackingGainVal').textContent='-8 dB';
})();
